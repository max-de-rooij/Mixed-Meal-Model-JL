...
function __pla(optf, calibrated_parameter_set, pla_parameter, loss_fnc, min_par, max_par, threshold, lb, ub, min_par_step, max_par_step, min_obj_change, max_obj_change, sample_size, direction, verbose)
    ...
    while continuation(pla_parameter, count, result, res, direction)
        ...
        if step == min_par_step && obj_change > max_obj_change
            verbose && println("maximum objective function change encountered with minimum step size.\nTemporarily raising max_obj_change from $(max_obj_change) to $(obj_change)")
...
PLAcompute(prob::PLAProblem, par_idx; min_par = :auto, max_par = :auto, threshold = :auto, lb = :auto, ub = :auto, 
    min_par_step = :auto, max_par_step = :auto, min_obj_change = :auto, max_obj_change = :auto, sample_size = 100, hyperparameters=nothing,
    verbose=true) = begin
    ...
    left_result = __pla(optf, calibrated_parameter_set, pla_parameter, loss_fnc, min_par, max_par, threshold, lb, ub, min_par_step, max_par_step, min_obj_change, max_obj_change, sample_size, :left, verbose)
    right_result =  __pla(optf, calibrated_parameter_set, pla_parameter, loss_fnc, min_par, max_par, threshold, lb, ub, min_par_step, max_par_step, min_obj_change, max_obj_change, sample_size, :right, verbose)
